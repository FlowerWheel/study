...
int main(int argc, char **argv)
{
	...
	{
		if ((ipv4_host = gethostbyname(argv[1])) == NULL)//域名解析
		{
	...
	errno = pthread_create(&send_tid, NULL, send_ping, NULL);
	if (errno != 0)
	{
		printf("send_ping thread fail\n");
		return -1;
	}
	errno = pthread_create(&recv_tid, NULL, recv_ping, NULL);
	if (errno != 0)
	{
		printf("recv_ping thread fail\n");
		return -1;
	}
	pthread_join(send_tid, &tret);
	pthread_join(recv_tid, &tret);
